...
namespace ModelGenerator.Test
{
    class Program
    {
        ...
        {
            var model = File.ReadAllLines(@"C:\Users\TYCHE\Source\Repos\ModelGenerator.Core\ModelGenerator.Core\Resources\typescript.fmt");
            var repo = File.ReadAllLines(@"C:\Users\TYCHE\Source\Repos\ModelGenerator.Core\ModelGenerator.Core\Resources\typescript_repo.fmt");
            var uow = File.ReadAllLines(@"C:\Users\TYCHE\Source\Repos\ModelGenerator.Core\ModelGenerator.Core\Resources\typescript_unitofwork.fmt");
            var repoBase = File.ReadAllLines(@"C:\Users\TYCHE\Source\Repos\ModelGenerator.Core\ModelGenerator.Core\Resources\typescript_repo_based.fmt");
            var baseDir = $@"C:\Users\TYCHE\Desktop\fmt";

            var genericGenerator = new GenericGenerator("server=localhost;database=local;user=sa;password=sa;", $@"{baseDir}\Models", @"Local.Models", model, x => $@"[{x}]");
            genericGenerator.SetDbConnector(new SqlConnection(), new SqlParameter());
            genericGenerator.Generate();
            genericGenerator.ChangeNamespace("Local");
